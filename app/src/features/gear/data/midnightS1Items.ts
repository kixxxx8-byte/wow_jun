import type { EquipmentSlotKey } from "../domain/gearTypes";
import type { SecondaryStat, SpecKey } from "../domain/specGuides";

export type SeasonItemSourceType = "dungeon" | "raid" | "craft" | "delve" | "catalyst" | "vendor" | "unknown";
export type ArmorType = "cloth" | "leather" | "mail" | "plate";
export type CuratedRecommendationState = "recommended" | "needs_check" | "hidden" | "rejected" | "db_missing";
export type TrinketTier = "S" | "A" | "B" | "C" | "주의";
export type TrinketTierContentFocus = "mythic_plus" | "raid" | "balanced";
export type ItemUpgradeTrack = "veteran" | "champion" | "hero" | "myth";
export type ItemVariantSource = "mythic_plus_end" | "great_vault" | "raid_lfr" | "raid_normal" | "raid_heroic" | "raid_mythic" | "crafted";

export type ItemSourceReference = {
  name: "Wowhead" | "SeasonLoot" | "Blizzard" | "Manual";
  url: string;
  checkedAt: string;
  noteKo?: string;
};

export type SeasonItemVariant = {
  variantId: string;
  itemId: number;
  itemLevel: number;
  track: ItemUpgradeTrack;
  rank: number;
  maxRank: number;
  source: ItemVariantSource;
  keyLevel?: number;
  difficultyKo?: string;
  confidence: "high" | "medium" | "low";
  sourceRef: "wowhead-midnight-mplus-rewards" | "wowhead-raid-boss-guide" | "manual";
};

export type TrinketTierRecord = {
  itemId: number;
  specKey: SpecKey;
  tier: TrinketTier;
  contentFocus: TrinketTierContentFocus;
  confidence: "high" | "medium" | "low";
  needsSim: boolean;
  summaryKo: string;
  sources: Array<{
    name: "Wowhead" | "Method" | "MythicSim" | "SeasonLoot" | "Manual";
    url?: string;
    checkedAt?: string;
  }>;
};

export type TierSetRecord = {
  setBonusKey: string;
  classKey: "rogue" | "demon-hunter";
  nameKo: string;
  nameEn: string;
  itemIds: number[];
  sourceRefs: ItemSourceReference[];
  bonusesKo: Array<{
    pieces: 2 | 4;
    specKey: SpecKey;
    summaryKo: string;
  }>;
};

export type SeasonItem = {
  itemId: number;
  nameKo: string;
  nameKoVerified: boolean;
  nameKoVerifiedAt?: string;
  nameKoSource?: "blizzard" | "manual";
  nameEn?: string;
  slot: EquipmentSlotKey;
  armorType?: ArmorType;
  weaponType?: "dagger" | "oneHandSword" | "oneHandAxe" | "warglaive" | "fistWeapon" | "staff" | "offHand" | "unknown";
  allowedClasses?: Array<"rogue" | "demon-hunter">;
  allowedSpecs?: SpecKey[];
  sourceType: SeasonItemSourceType;
  sourceNameKo: string;
  sourceNameEn?: string;
  sourceDungeonKey?: string;
  sourceRaidKey?: string;
  bossNameKo?: string;
  bossNameEn?: string;
  stats?: Partial<Record<SecondaryStat, true>>;
  recommendedStats?: SecondaryStat[];
  hasSpecialEffect?: boolean;
  specialEffectNote?: string;
  isTierPiece?: boolean;
  tierToken?: string;
  isCrafted?: boolean;
  professionKo?: string;
  embellishmentAllowed?: boolean;
  embellishmentNameKo?: string;
  wowheadUrl?: string;
  sourceRefs?: ItemSourceReference[];
  variants?: SeasonItemVariant[];
  uniqueEquipGroup?: string;
  setBonusKey?: string;
  isSeasonalReward?: boolean;
  season: "midnight-s1";
  confidence: "high" | "medium" | "low";
  recommendationState: CuratedRecommendationState;
  trinketTier?: TrinketTierRecord;
  tags?: Array<"bisCandidate" | "strongCandidate" | "backupCandidate" | "statStick" | "specialEffect" | "craftedOption" | "needsSim">;
  note?: string;
};

export const midnightS1MythicPlusItemLevelTable = [
  { keyLevel: 2, endItemLevel: 250, endTrack: "champion", endRank: 2, vaultItemLevel: 259, vaultTrack: "hero", vaultRank: 1 },
  { keyLevel: 3, endItemLevel: 250, endTrack: "champion", endRank: 2, vaultItemLevel: 259, vaultTrack: "hero", vaultRank: 1 },
  { keyLevel: 4, endItemLevel: 253, endTrack: "champion", endRank: 3, vaultItemLevel: 263, vaultTrack: "hero", vaultRank: 2 },
  { keyLevel: 5, endItemLevel: 256, endTrack: "champion", endRank: 4, vaultItemLevel: 263, vaultTrack: "hero", vaultRank: 2 },
  { keyLevel: 6, endItemLevel: 259, endTrack: "hero", endRank: 1, vaultItemLevel: 266, vaultTrack: "hero", vaultRank: 3 },
  { keyLevel: 7, endItemLevel: 259, endTrack: "hero", endRank: 1, vaultItemLevel: 269, vaultTrack: "hero", vaultRank: 4 },
  { keyLevel: 8, endItemLevel: 263, endTrack: "hero", endRank: 2, vaultItemLevel: 269, vaultTrack: "hero", vaultRank: 4 },
  { keyLevel: 9, endItemLevel: 263, endTrack: "hero", endRank: 2, vaultItemLevel: 269, vaultTrack: "hero", vaultRank: 4 },
  { keyLevel: 10, endItemLevel: 266, endTrack: "hero", endRank: 3, vaultItemLevel: 272, vaultTrack: "myth", vaultRank: 1 },
  { keyLevel: 11, endItemLevel: 266, endTrack: "hero", endRank: 3, vaultItemLevel: 272, vaultTrack: "myth", vaultRank: 1 },
  { keyLevel: 12, endItemLevel: 266, endTrack: "hero", endRank: 3, vaultItemLevel: 272, vaultTrack: "myth", vaultRank: 1 },
] as const;

export const midnightS1RaidItemLevelTable = [
  { difficultyKo: "공격대 찾기", itemLevel: 240, track: "veteran", rank: 3 },
  { difficultyKo: "일반", itemLevel: 253, track: "champion", rank: 3 },
  { difficultyKo: "영웅", itemLevel: 266, track: "hero", rank: 3 },
  { difficultyKo: "신화", itemLevel: 279, track: "myth", rank: 3 },
] as const;

const mythicPlusVariants = (itemId: number): SeasonItemVariant[] => midnightS1MythicPlusItemLevelTable.flatMap((row) => [
  {
    variantId: `${itemId}:mplus-${row.keyLevel}-end`,
    itemId,
    itemLevel: row.endItemLevel,
    track: row.endTrack,
    rank: row.endRank,
    maxRank: 6,
    source: "mythic_plus_end",
    keyLevel: row.keyLevel,
    confidence: "high",
    sourceRef: "wowhead-midnight-mplus-rewards",
  },
  {
    variantId: `${itemId}:mplus-${row.keyLevel}-vault`,
    itemId,
    itemLevel: row.vaultItemLevel,
    track: row.vaultTrack,
    rank: row.vaultRank,
    maxRank: 6,
    source: "great_vault",
    keyLevel: row.keyLevel,
    confidence: "high",
    sourceRef: "wowhead-midnight-mplus-rewards",
  },
]);

const raidVariants = (itemId: number): SeasonItemVariant[] => midnightS1RaidItemLevelTable.map((row) => ({
  variantId: `${itemId}:raid-${row.track}`,
  itemId,
  itemLevel: row.itemLevel,
  track: row.track,
  rank: row.rank,
  maxRank: 6,
  source: row.track === "veteran" ? "raid_lfr" : row.track === "champion" ? "raid_normal" : row.track === "hero" ? "raid_heroic" : "raid_mythic",
  difficultyKo: row.difficultyKo,
  confidence: "medium",
  sourceRef: "wowhead-raid-boss-guide",
}));

const wowheadItemRef = (itemId: number, checkedAt = "2026-06-10"): ItemSourceReference => ({
  name: "Wowhead",
  url: `https://www.wowhead.com/item=${itemId}`,
  checkedAt,
});

const seasonLootRef: ItemSourceReference = {
  name: "SeasonLoot",
  url: "https://seasonloot.com/rogue/",
  checkedAt: "2026-06-10",
  noteKo: "도적용 전체 슬롯/출처 목록 대조용입니다. itemId는 Wowhead 개별 페이지로 확인합니다.",
};

const rogueTierSetRef: ItemSourceReference = {
  name: "Wowhead",
  url: "https://www.wowhead.com/item-set=1987/motley-of-the-grim-jest",
  checkedAt: "2026-06-10",
  noteKo: "도적 한밤 시즌1 티어 세트 효과와 구성 부위 대조용입니다.",
};

const devourerTierSetRef: ItemSourceReference = {
  name: "Wowhead",
  url: "https://www.wowhead.com/item-set=1979/devouring-reavers-sheathe",
  checkedAt: "2026-06-10",
  noteKo: "악마사냥꾼 한밤 시즌1 티어 세트 효과와 구성 부위 대조용입니다.",
};

export const trinketTierRecords: TrinketTierRecord[] = [
  {
    itemId: 249343,
    specKey: "rogue-assassination",
    tier: "주의",
    contentFocus: "raid",
    confidence: "medium",
    needsSim: true,
    summaryKo: "레이드 출처 비교 후보입니다. 레이드 제외/던전+제작 기준에서는 기본 추천하지 않고, 실제 장신구 교체는 SimC 확인이 필요합니다.",
    sources: [
      { name: "Manual", checkedAt: "2026-05-20" },
      { name: "Wowhead", url: "https://www.wowhead.com/ko/guide/classes/rogue/assassination/bis-gear", checkedAt: "2026-05-20" },
    ],
  },
];

export const tierSetRecords: TierSetRecord[] = [
  {
    setBonusKey: "motley-of-the-grim-jest",
    classKey: "rogue",
    nameKo: "음산한 농담꾼의 의복",
    nameEn: "Motley of the Grim Jest",
    itemIds: [250006, 250004, 250009, 250007, 250005],
    sourceRefs: [rogueTierSetRef],
    bonusesKo: [
      {
        pieces: 2,
        specKey: "rogue-assassination",
        summaryKo: "목조르기 피해 증가 및 독 적용 관련 2세트 효과입니다.",
      },
      {
        pieces: 4,
        specKey: "rogue-assassination",
        summaryKo: "목조르기 기력 비용 감소와 독 대상 공격 강화 관련 4세트 효과입니다.",
      },
      {
        pieces: 2,
        specKey: "rogue-outlaw",
        summaryKo: "Blade Rush 피해 증가와 주 대상 추가 피해 관련 2세트 효과입니다.",
      },
      {
        pieces: 4,
        specKey: "rogue-outlaw",
        summaryKo: "Blade Rush 재사용 대기시간 감소와 짧은 피해 증가 관련 4세트 효과입니다.",
      },
      {
        pieces: 2,
        specKey: "rogue-subtlety",
        summaryKo: "마무리 일격이 사용한 연계 점수에 따라 추가 피해를 주는 2세트 효과입니다.",
      },
      {
        pieces: 4,
        specKey: "rogue-subtlety",
        summaryKo: "어둠의 칼날 지속시간 증가와 추가 암흑 피해 관련 4세트 효과입니다.",
      },
    ],
  },
  {
    setBonusKey: "devouring-reavers-sheathe",
    classKey: "demon-hunter",
    nameKo: "포식하는 약탈자의 칼집",
    nameEn: "Devouring Reaver's Sheathe",
    itemIds: [250033, 250031, 250036, 250034, 250032],
    sourceRefs: [devourerTierSetRef],
    bonusesKo: [
      {
        pieces: 2,
        specKey: "demon-hunter-devourer",
        summaryKo: "공허 광선 피해가 증가하는 Devourer 2세트 효과입니다.",
      },
      {
        pieces: 4,
        specKey: "demon-hunter-devourer",
        summaryKo: "붕괴하는 별 피해 증가와 격노 생성이 붙는 Devourer 4세트 효과입니다.",
      },
    ],
  },
];

export const midnightS1Items: SeasonItem[] = [
  {
    itemId: 237837,
    nameKo: "원정순찰대원의 자비",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    slot: "OFF_HAND",
    weaponType: "offHand",
    allowedClasses: ["rogue"],
    sourceType: "craft",
    sourceNameKo: "전문기술 제작",
    isCrafted: true,
    professionKo: "제작 주문",
    recommendedStats: ["crit", "haste"],
    embellishmentAllowed: true,
    season: "midnight-s1",
    confidence: "high",
    recommendationState: "recommended",
    tags: ["craftedOption", "strongCandidate"],
    note: "현재 프로젝트에서 수동 검수된 제작 보조무기 후보입니다. 이미 장착 중이면 추천에서 제외합니다.",
  },
  {
    itemId: 244576,
    nameKo: "실버문 요원의 굴절보호대",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    slot: "WRIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "craft",
    sourceNameKo: "전문기술 제작",
    isCrafted: true,
    professionKo: "가죽세공",
    recommendedStats: ["mastery", "haste"],
    embellishmentAllowed: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["craftedOption"],
    note: "제작 전 현재 손목 마법부여와 대체 후보를 함께 확인합니다.",
  },
  {
    itemId: 249343,
    nameKo: "알른 선견자의 응시",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    slot: "TRINKET_1",
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드",
    sourceRaidKey: "midnight-s1-raid",
    bossNameKo: "키마이루스",
    hasSpecialEffect: true,
    specialEffectNote: "장신구는 단순 보조 능력치 점수로 확정하지 않고 실제 효과와 시뮬레이션을 확인합니다.",
    wowheadUrl: "https://www.wowhead.com/item=249343",
    sourceRefs: [wowheadItemRef(249343)],
    variants: raidVariants(249343),
    uniqueEquipGroup: "trinket-249343",
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      ...trinketTierRecords[0],
      tier: "주의",
      contentFocus: "raid",
      confidence: "medium",
      needsSim: true,
      summaryKo: "레이드 출처 비교 후보입니다. 레이드 제외/던전+제작 기준에서는 기본 추천하지 않고, 실제 장신구 교체는 SimC 확인이 필요합니다.",
      sources: [
        { name: "Manual", checkedAt: "2026-05-20" },
        { name: "Wowhead", url: "https://www.wowhead.com/ko/guide/classes/rogue/assassination/bis-gear", checkedAt: "2026-05-20" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "레이드 제외 모드에서는 추천하지 않는 비교 후보입니다.",
  },
  {
    itemId: 251109,
    nameKo: "주문절단 그림자복면",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Spellsnap Shadowmask",
    slot: "HEAD",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마법학자의 정원",
    sourceNameEn: "Magisters' Terrace",
    sourceDungeonKey: "magisters",
    bossNameKo: "세라넬 선래시",
    bossNameEn: "Seranel Sunlash",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251109",
    sourceRefs: [wowheadItemRef(251109), seasonLootRef],
    variants: mythicPlusVariants(251109),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 아이템 페이지 기준으로 itemId와 출처를 확인했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251177,
    nameKo: "악취 나는 부정왕관",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Fetid Vilecrown",
    slot: "HEAD",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마이사라 동굴",
    sourceNameEn: "Maisara Caverns",
    sourceDungeonKey: "maisara",
    bossNameKo: "라크툴",
    bossNameEn: "Rak'tul",
    stats: { crit: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251177",
    sourceRefs: [wowheadItemRef(251177), seasonLootRef],
    variants: mythicPlusVariants(251177),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 아이템 페이지 기준으로 itemId와 출처를 확인했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251216,
    nameKo: "악독한 조끼",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Maledict Vest",
    slot: "CHEST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "연결지점 제나스",
    sourceNameEn: "Nexus-Point Xenas",
    sourceDungeonKey: "xenas",
    bossNameKo: "로트락시온",
    bossNameEn: "Lothraxion",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251216",
    sourceRefs: [wowheadItemRef(251216), seasonLootRef],
    variants: mythicPlusVariants(251216),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 아이템 페이지 기준으로 itemId와 출처를 확인했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251082,
    nameKo: "치악덩굴 허리끈",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Snapvine Cinch",
    slot: "WAIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "엠버돈",
    bossNameEn: "Emberdawn",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251082",
    sourceRefs: [wowheadItemRef(251082), seasonLootRef],
    variants: mythicPlusVariants(251082),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 아이템 페이지 기준으로 itemId와 출처를 확인했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251087,
    nameKo: "머무는 유산의 다리싸개",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Legwraps of Lingering Legacies",
    slot: "LEGS",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "칼리스",
    bossNameEn: "Kalis",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=251087",
    sourceRefs: [wowheadItemRef(251087), seasonLootRef],
    variants: mythicPlusVariants(251087),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 아이템 페이지 기준으로 itemId와 출처를 확인했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251212,
    nameKo: "찬란한 분리검",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Radiant Slicer",
    slot: "MAIN_HAND",
    weaponType: "dagger",
    allowedClasses: ["rogue"],
    sourceType: "dungeon",
    sourceNameKo: "연결지점 제나스",
    sourceNameEn: "Nexus-Point Xenas",
    sourceDungeonKey: "xenas",
    bossNameKo: "로트락시온",
    bossNameEn: "Lothraxion",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251212",
    sourceRefs: [wowheadItemRef(251212), seasonLootRef],
    variants: mythicPlusVariants(251212),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "도적 전용 무기 후보입니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250256,
    nameKo: "바람의 심장",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Heart of Wind",
    slot: "TRINKET_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "불안정한 심장",
    bossNameEn: "Restless Heart",
    hasSpecialEffect: true,
    specialEffectNote: "가속 관련 발동형 장신구로 보이며, 직업/특성별 실제 가치는 SimC 확인이 필요합니다.",
    wowheadUrl: "https://www.wowhead.com/item=250256",
    sourceRefs: [wowheadItemRef(250256), seasonLootRef],
    variants: mythicPlusVariants(250256),
    uniqueEquipGroup: "trinket-250256",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 250256,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 장신구 후보입니다. 발동 효과 중심이라 전문화별 SimC 확인 전에는 확정 추천하지 않습니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=250256", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "장신구는 티어/효과/실제 전투 상황 차이가 커서 기본 추천이 아니라 확인 후보로만 표시합니다.",
  },
  {
    itemId: 250227,
    nameKo: "크롤루크의 전쟁 깃발",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Kroluk's Warbanner",
    slot: "TRINKET_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "사령관 크롤루크",
    bossNameEn: "Commander Kroluk",
    hasSpecialEffect: true,
    specialEffectNote: "근접 공격 기반 발동형 장신구로 보이며, 전문화별 실제 가치는 SimC 확인이 필요합니다.",
    wowheadUrl: "https://www.wowhead.com/item=250227",
    sourceRefs: [wowheadItemRef(250227), seasonLootRef],
    variants: mythicPlusVariants(250227),
    uniqueEquipGroup: "trinket-250227",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 250227,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 장신구 후보입니다. 근접 발동 효과라 전문화/전투 시간에 따라 가치가 크게 달라질 수 있습니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=250227", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "장신구는 티어/효과/실제 전투 상황 차이가 커서 기본 추천이 아니라 확인 후보로만 표시합니다.",
  },
  {
    itemId: 251096,
    nameKo: "괴로운 비탄의 펜던트",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Pendant of Aching Grief",
    slot: "NECK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "불안정한 심장",
    bossNameEn: "Restless Heart",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251096",
    sourceRefs: [wowheadItemRef(251096), seasonLootRef],
    variants: mythicPlusVariants(251096),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 50228,
    nameKo: "뾰족한 이미르하임 목장식",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Barbed Ymirheim Choker",
    slot: "NECK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "제련장인 가프로스트",
    bossNameEn: "Forgemaster Garfrost",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=50228",
    sourceRefs: [wowheadItemRef(50228), seasonLootRef],
    variants: mythicPlusVariants(50228),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "과거 던전 원본 아이템이지만 SeasonLoot 한밤 S1 목록에 포함되어 현재 시즌 variant로만 비교합니다.",
  },
  {
    itemId: 151319,
    nameKo: "황혼의 서슬 어깨덮개",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Twilight's Edge Spaulders",
    slot: "SHOULDER",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "삼두정의 권좌",
    sourceNameEn: "Seat of the Triumvirate",
    sourceDungeonKey: "seat",
    bossNameKo: "루라",
    bossNameEn: "L'ura",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=151319",
    sourceRefs: [wowheadItemRef(151319), seasonLootRef],
    variants: mythicPlusVariants(151319),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "과거 던전 원본 아이템이지만 SeasonLoot 한밤 S1 목록에 포함되어 현재 시즌 variant로만 비교합니다.",
  },
  {
    itemId: 258581,
    nameKo: "붉은깃털 어깨덧옷",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Bloodfeather Mantle",
    slot: "SHOULDER",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "하늘탑",
    sourceNameEn: "Skyreach",
    sourceDungeonKey: "skyreach",
    bossNameKo: "루크란",
    bossNameEn: "Rukhran",
    stats: { mastery: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=258581",
    sourceRefs: [wowheadItemRef(258581), seasonLootRef],
    variants: mythicPlusVariants(258581),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251171,
    nameKo: "마법에 걸린 해골가시",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Enthralled Bonespines",
    slot: "SHOULDER",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마이사라 동굴",
    sourceNameEn: "Maisara Caverns",
    sourceDungeonKey: "maisara",
    bossNameKo: "보르다자",
    bossNameEn: "Vordaza",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251171",
    sourceRefs: [wowheadItemRef(251171), seasonLootRef],
    variants: mythicPlusVariants(251171),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 260312,
    nameKo: "저항하는 수호자의 외투",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Defiant Defender's Drape",
    slot: "BACK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마법학자의 정원",
    sourceNameEn: "Magisters' Terrace",
    sourceDungeonKey: "magisters",
    bossNameKo: "세라넬 선래시",
    bossNameEn: "Seranel Sunlash",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=260312",
    sourceRefs: [wowheadItemRef(260312), seasonLootRef],
    variants: mythicPlusVariants(260312),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 133505,
    nameKo: "쓰러진 추기경의 망토",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Cloak of the Fallen Cardinal",
    slot: "BACK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "스컬지군주 티라누스",
    bossNameEn: "Scourgelord Tyrannus",
    stats: { crit: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=133505",
    sourceRefs: [wowheadItemRef(133505), seasonLootRef],
    variants: mythicPlusVariants(133505),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "과거 던전 원본 아이템이지만 SeasonLoot 한밤 S1 목록에 포함되어 현재 시즌 variant로만 비교합니다.",
  },
  {
    itemId: 258575,
    nameKo: "강도 높은 미늘 큰망토",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Rigid Scale Greatcloak",
    slot: "BACK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "하늘탑",
    sourceNameEn: "Skyreach",
    sourceDungeonKey: "skyreach",
    bossNameKo: "란지트",
    bossNameEn: "Ranjit",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=258575",
    sourceRefs: [wowheadItemRef(258575), seasonLootRef],
    variants: mythicPlusVariants(258575),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251204,
    nameKo: "핵장인의 제어 장치",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Corewright's Zappers",
    slot: "HANDS",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "연결지점 제나스",
    sourceNameEn: "Nexus-Point Xenas",
    sourceDungeonKey: "xenas",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251204",
    sourceRefs: [wowheadItemRef(251204), seasonLootRef],
    variants: mythicPlusVariants(251204),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Wowhead 개별 페이지에서 boss 정보가 명확히 노출되지 않아 출처 던전만 검수했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251121,
    nameKo: "도마나르의 광포한 발보호대",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Domanaar's Dire Treads",
    slot: "FEET",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마법학자의 정원",
    sourceNameEn: "Magisters' Terrace",
    sourceDungeonKey: "magisters",
    bossNameKo: "데겐트리우스",
    bossNameEn: "Degentrius",
    stats: { mastery: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251121",
    sourceRefs: [wowheadItemRef(251121), seasonLootRef],
    variants: mythicPlusVariants(251121),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 193712,
    nameKo: "물약으로 얼룩진 망토",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Potion-Stained Cloak",
    slot: "BACK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "알게타르 대학",
    sourceNameEn: "Algeth'ar Academy",
    sourceDungeonKey: "algethar",
    bossNameKo: "우거진 고대정령",
    bossNameEn: "Overgrown Ancient",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=193712",
    sourceRefs: [wowheadItemRef(193712), seasonLootRef],
    variants: mythicPlusVariants(193712),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "알게타르 대학 망토 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 193714,
    nameKo: "광란뿌리 소매장식",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Frenzyroot Cuffs",
    slot: "WRIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "알게타르 대학",
    sourceNameEn: "Algeth'ar Academy",
    sourceDungeonKey: "algethar",
    bossNameKo: "우거진 고대정령",
    bossNameEn: "Overgrown Ancient",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=193714",
    sourceRefs: [wowheadItemRef(193714), seasonLootRef],
    variants: mythicPlusVariants(193714),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["strongCandidate"],
    note: "알게타르 대학 가죽 손목 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 193721,
    nameKo: "루비 참가자의 장갑",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Ruby Contestant's Gloves",
    slot: "HANDS",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "알게타르 대학",
    sourceNameEn: "Algeth'ar Academy",
    sourceDungeonKey: "algethar",
    bossNameKo: "크로스",
    bossNameEn: "Crawth",
    stats: { mastery: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=193721",
    sourceRefs: [wowheadItemRef(193721), seasonLootRef],
    variants: mythicPlusVariants(193721),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "알게타르 대학 가죽 손 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 50264,
    nameKo: "물어뜯긴 가죽 손목보호구",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Chewed Leather Wristguards",
    slot: "WRIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "이크",
    bossNameEn: "Ick",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=50264",
    sourceRefs: [wowheadItemRef(50264), seasonLootRef],
    variants: mythicPlusVariants(50264),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["strongCandidate"],
    note: "사론의 구덩이 가죽 손목 후보입니다. 과거 원본 던전템이므로 현재 시즌 variant 기준으로만 비교합니다.",
  },
  {
    itemId: 49806,
    nameKo: "바위갈퀴의 검은띠",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Flayer's Black Belt",
    slot: "WAIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "제련장인 가프로스트",
    bossNameEn: "Forgemaster Garfrost",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=49806",
    sourceRefs: [wowheadItemRef(49806), seasonLootRef],
    variants: mythicPlusVariants(49806),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["strongCandidate"],
    note: "사론의 구덩이 가죽 허리 후보입니다. 과거 원본 던전템이므로 현재 시즌 variant 기준으로만 비교합니다.",
  },
  {
    itemId: 49817,
    nameKo: "털이 많은 고룡가죽 다리보호구",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Shaggy Wyrmleather Leggings",
    slot: "LEGS",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "스컬지군주 티라누스",
    bossNameEn: "Scourgelord Tyrannus",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=49817",
    sourceRefs: [wowheadItemRef(49817), seasonLootRef],
    variants: mythicPlusVariants(49817),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "사론의 구덩이 가죽 다리 후보입니다. 과거 원본 던전템이므로 현재 시즌 variant 기준으로만 비교합니다.",
  },
  {
    itemId: 49812,
    nameKo: "훔친 결혼반지",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Purloined Wedding Ring",
    slot: "FINGER_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "사론의 구덩이",
    sourceNameEn: "Pit of Saron",
    sourceDungeonKey: "saron",
    bossNameKo: "이크",
    bossNameEn: "Ick",
    stats: { crit: true, haste: true },
    wowheadUrl: "https://www.wowhead.com/item=49812",
    sourceRefs: [wowheadItemRef(49812), seasonLootRef],
    variants: mythicPlusVariants(49812),
    uniqueEquipGroup: "ring-49812",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "과거 던전 원본 아이템이지만 SeasonLoot 한밤 S1 목록에 포함되어 현재 시즌 variant로만 비교합니다.",
  },
  {
    itemId: 251115,
    nameKo: "분기점의 고리",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Bifurcation Band",
    slot: "FINGER_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마법학자의 정원",
    sourceNameEn: "Magisters' Terrace",
    sourceDungeonKey: "magisters",
    bossNameKo: "게멜루스",
    bossNameEn: "Gemellus",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251115",
    sourceRefs: [wowheadItemRef(251115), seasonLootRef],
    variants: mythicPlusVariants(251115),
    uniqueEquipGroup: "ring-251115",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "SeasonLoot 슬롯 목록과 Wowhead 개별 페이지를 대조했습니다. 한국어 공식 아이템명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 151308,
    nameKo: "에레다스 귀족의 인장",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Eredath Seal of Nobility",
    slot: "FINGER_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "삼두정의 권좌",
    sourceNameEn: "Seat of the Triumvirate",
    sourceDungeonKey: "seat",
    bossNameKo: "주라알",
    bossNameEn: "Zuraal the Ascended",
    stats: { crit: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=151308",
    sourceRefs: [wowheadItemRef(151308), seasonLootRef],
    variants: mythicPlusVariants(151308),
    uniqueEquipGroup: "ring-151308",
    setBonusKey: "velens-lament",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Velen's Lament 세트 반지입니다. 세트 효과 가치는 별도 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 151311,
    nameKo: "삼두정의 고리",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Band of the Triumvirate",
    slot: "FINGER_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "삼두정의 권좌",
    sourceNameEn: "Seat of the Triumvirate",
    sourceDungeonKey: "seat",
    bossNameKo: "루라",
    bossNameEn: "L'ura",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=151311",
    sourceRefs: [wowheadItemRef(151311), seasonLootRef],
    variants: mythicPlusVariants(151311),
    uniqueEquipGroup: "ring-151311",
    setBonusKey: "velens-lament",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "Velen's Lament 세트 반지입니다. 세트 효과 가치는 별도 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 251093,
    nameKo: "소외된 빛",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Omission of Light",
    slot: "FINGER_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "연결지점 제나스",
    sourceNameEn: "Nexus-Point Xenas",
    sourceDungeonKey: "xenas",
    bossNameKo: "핵수호자 나이사라",
    bossNameEn: "Corewarden Nysarra",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251093",
    sourceRefs: [wowheadItemRef(251093), seasonLootRef],
    variants: mythicPlusVariants(251093),
    uniqueEquipGroup: "ring-251093",
    setBonusKey: "voidlight-bindings",
    hasSpecialEffect: true,
    specialEffectNote: "Voidlight Bindings 세트 효과는 전투 상황에 따라 가치가 달라질 수 있어 별도 검수가 필요합니다.",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate", "needsSim"],
    note: "Voidlight Bindings 세트 반지입니다. 세트 효과 가치는 별도 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 193708,
    nameKo: "백금 별의 고리",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Platinum Star Band",
    slot: "FINGER_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "알게타르 대학",
    sourceNameEn: "Algeth'ar Academy",
    sourceDungeonKey: "algethar",
    bossNameKo: "벡사무스",
    bossNameEn: "Vexamus",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=193708",
    sourceRefs: [wowheadItemRef(193708), seasonLootRef],
    variants: mythicPlusVariants(193708),
    uniqueEquipGroup: "ring-193708",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "알게타르 대학 반지 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 252420,
    nameKo: "태양섬광 분광경",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Solarflare Prism",
    slot: "TRINKET_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "하늘탑",
    sourceNameEn: "Skyreach",
    sourceDungeonKey: "skyreach",
    bossNameKo: "대현자 비릭스",
    bossNameEn: "High Sage Viryx",
    hasSpecialEffect: true,
    specialEffectNote: "치명타 관련 발동형 장신구로 보이며, 전문화별 실제 가치는 SimC 확인이 필요합니다.",
    wowheadUrl: "https://www.wowhead.com/item=252420",
    sourceRefs: [wowheadItemRef(252420), seasonLootRef],
    variants: mythicPlusVariants(252420),
    uniqueEquipGroup: "trinket-252420",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 252420,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 장신구 후보입니다. 발동 효과와 대상 체력 조건 때문에 SimC 확인 전에는 확정 추천하지 않습니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=252420", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "장신구는 티어/효과/실제 전투 상황 차이가 커서 기본 추천이 아니라 확인 후보로만 표시합니다.",
  },
  {
    itemId: 250258,
    nameKo: "괴로워하는 영혼의 그릇",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Vessel of Tortured Souls",
    slot: "TRINKET_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마이사라 동굴",
    sourceNameEn: "Maisara Caverns",
    sourceDungeonKey: "maisara",
    bossNameKo: "라크툴",
    bossNameEn: "Rak'tul",
    stats: { mastery: true },
    hasSpecialEffect: true,
    specialEffectNote: "영혼 중첩형 장신구로 보이며, 실제 가치는 전투 시간과 중첩 유지에 따라 달라집니다.",
    wowheadUrl: "https://www.wowhead.com/item=250258",
    sourceRefs: [wowheadItemRef(250258), seasonLootRef],
    variants: mythicPlusVariants(250258),
    uniqueEquipGroup: "trinket-250258",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 250258,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 장신구 후보입니다. 중첩형 효과라 전투 길이와 타겟 전환에 따라 가치가 달라질 수 있습니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=250258", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "장신구는 티어/효과/실제 전투 상황 차이가 커서 기본 추천이 아니라 확인 후보로만 표시합니다.",
  },
  {
    itemId: 193701,
    nameKo: "알게타르 수수께끼 상자",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Algeth'ar Puzzle Box",
    slot: "TRINKET_1",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "알게타르 대학",
    sourceNameEn: "Algeth'ar Academy",
    sourceDungeonKey: "algethar",
    bossNameKo: "도라고사",
    bossNameEn: "Echo of Doragosa",
    hasSpecialEffect: true,
    specialEffectNote: "사용 효과로 특화를 크게 올리는 장신구입니다. 사용 타이밍과 시전/이동 제약은 실제 전투와 SimC 확인이 필요합니다.",
    wowheadUrl: "https://www.wowhead.com/item=193701",
    sourceRefs: [wowheadItemRef(193701), seasonLootRef],
    variants: mythicPlusVariants(193701),
    uniqueEquipGroup: "trinket-193701",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 193701,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 사용형 특화 장신구 후보입니다. 실제 가치는 쿨기 정렬, 시전 타이밍, 전투 길이에 따라 달라져 SimC 확인이 필요합니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=193701", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "알게타르 대학 장신구 후보입니다. 현재 시즌 보상 테이블 확인 후보로 등록하되 장신구는 확정 추천하지 않습니다.",
  },
  {
    itemId: 250144,
    nameKo: "잿불날개 깃털",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Emberwing Feather",
    slot: "TRINKET_2",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "윈드러너 첨탑",
    sourceNameEn: "Windrunner Spire",
    sourceDungeonKey: "windrunner",
    bossNameKo: "엠버돈",
    bossNameEn: "Emberdawn",
    hasSpecialEffect: true,
    specialEffectNote: "사용 효과로 가속을 올리지만 다른 보조 능력치 감소 가능성이 있어 실제 가치는 SimC 확인이 필요합니다.",
    wowheadUrl: "https://www.wowhead.com/item=250144",
    sourceRefs: [wowheadItemRef(250144), seasonLootRef],
    variants: mythicPlusVariants(250144),
    uniqueEquipGroup: "trinket-250144",
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    trinketTier: {
      itemId: 250144,
      specKey: "rogue-outlaw",
      tier: "주의",
      contentFocus: "mythic_plus",
      confidence: "medium",
      needsSim: true,
      summaryKo: "쐐기 출처 사용형 장신구 후보입니다. 가속 증가와 보조 능력치 감소 위험이 있어 SimC 확인이 필요합니다.",
      sources: [
        { name: "Wowhead", url: "https://www.wowhead.com/item=250144", checkedAt: "2026-06-10" },
        { name: "SeasonLoot", url: "https://seasonloot.com/rogue/", checkedAt: "2026-06-10" },
      ],
    },
    tags: ["specialEffect", "needsSim"],
    note: "장신구는 티어/효과/실제 전투 상황 차이가 커서 기본 추천이 아니라 확인 후보로만 표시합니다.",
  },
  {
    itemId: 251166,
    nameKo: "매사냥꾼의 허리끈",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Falconer's Cinch",
    slot: "WAIST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마이사라 동굴",
    sourceNameEn: "Maisara Caverns",
    sourceDungeonKey: "maisara",
    bossNameKo: "무로진",
    bossNameEn: "Muro'jin",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251166",
    sourceRefs: [wowheadItemRef(251166), seasonLootRef],
    variants: mythicPlusVariants(251166),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "마이사라 동굴 가죽 허리 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 251161,
    nameKo: "영혼사냥꾼의 장막",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Shroud of the Soulhunter",
    slot: "BACK",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마이사라 동굴",
    sourceNameEn: "Maisara Caverns",
    sourceDungeonKey: "maisara",
    bossNameKo: "보르다자",
    bossNameEn: "Vordaza",
    stats: { mastery: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251161",
    sourceRefs: [wowheadItemRef(251161), seasonLootRef],
    variants: mythicPlusVariants(251161),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "마이사라 동굴 망토 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 151313,
    nameKo: "공허의 포옹 조끼",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Vest of the Void's Embrace",
    slot: "CHEST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "삼두정의 권좌",
    sourceNameEn: "Seat of the Triumvirate",
    sourceDungeonKey: "seat",
    bossNameKo: "루라",
    bossNameEn: "L'ura",
    stats: { crit: true, versatility: true },
    hasSpecialEffect: true,
    specialEffectNote: "암흑 피해를 받을 때 생기흡수 증가 효과가 있어 실제 가치는 전투 패턴에 따라 달라질 수 있습니다.",
    wowheadUrl: "https://www.wowhead.com/item=151313",
    sourceRefs: [wowheadItemRef(151313), seasonLootRef],
    variants: mythicPlusVariants(151313),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate", "needsSim"],
    note: "삼두정의 권좌 가죽 가슴 후보입니다. 과거 원본 던전템이므로 현재 시즌 variant 기준으로만 비교합니다.",
  },
  {
    itemId: 151314,
    nameKo: "변화의 추적자 가죽 바지",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Shifting Stalker Hide Pants",
    slot: "LEGS",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "삼두정의 권좌",
    sourceNameEn: "Seat of the Triumvirate",
    sourceDungeonKey: "seat",
    bossNameKo: "사프리쉬",
    bossNameEn: "Saprish",
    stats: { mastery: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=151314",
    sourceRefs: [wowheadItemRef(151314), seasonLootRef],
    variants: mythicPlusVariants(151314),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "삼두정의 권좌 가죽 다리 후보입니다. 과거 원본 던전템이므로 현재 시즌 variant 기준으로만 비교합니다.",
  },
  {
    itemId: 258577,
    nameKo: "타오르는 집중의 장화",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Boots of Burning Focus",
    slot: "FEET",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "하늘탑",
    sourceNameEn: "Skyreach",
    sourceDungeonKey: "skyreach",
    bossNameKo: "아라크나스",
    bossNameEn: "Araknath",
    stats: { crit: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=258577",
    sourceRefs: [wowheadItemRef(258577), seasonLootRef],
    variants: mythicPlusVariants(258577),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "하늘탑 가죽 발 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 258586,
    nameKo: "붉은깃털 가슴보호대",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Bloodfeather Chestguard",
    slot: "CHEST",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "하늘탑",
    sourceNameEn: "Skyreach",
    sourceDungeonKey: "skyreach",
    bossNameKo: "대현자 비릭스",
    bossNameEn: "High Sage Viryx",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=258586",
    sourceRefs: [wowheadItemRef(258586), seasonLootRef],
    variants: mythicPlusVariants(258586),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "하늘탑 가죽 가슴 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 251122,
    nameKo: "어둠칼날 절단기",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Shadowslash Slicer",
    slot: "MAIN_HAND",
    weaponType: "oneHandSword",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "마법학자의 정원",
    sourceNameEn: "Magisters' Terrace",
    sourceDungeonKey: "magisters",
    bossNameKo: "데겐트리우스",
    bossNameEn: "Degentrius",
    stats: { haste: true, mastery: true },
    wowheadUrl: "https://www.wowhead.com/item=251122",
    sourceRefs: [wowheadItemRef(251122), seasonLootRef],
    variants: mythicPlusVariants(251122),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "마법학자의 정원 한손 도검 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 251210,
    nameKo: "일월식 발목화",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Eclipse Espadrilles",
    slot: "FEET",
    armorType: "leather",
    allowedClasses: ["rogue", "demon-hunter"],
    sourceType: "dungeon",
    sourceNameKo: "연결지점 제나스",
    sourceNameEn: "Nexus-Point Xenas",
    sourceDungeonKey: "xenas",
    bossNameKo: "핵수호자 나이사라",
    bossNameEn: "Corewarden Nysarra",
    stats: { haste: true, versatility: true },
    wowheadUrl: "https://www.wowhead.com/item=251210",
    sourceRefs: [wowheadItemRef(251210), seasonLootRef],
    variants: mythicPlusVariants(251210),
    isSeasonalReward: true,
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["backupCandidate"],
    note: "연결지점 제나스 가죽 발 후보입니다. 한국어 공식명 검수 전까지 확인 후보로만 표시합니다.",
  },
  {
    itemId: 250006,
    nameKo: "암담한 재담의 가면",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Masquerade of the Grim Jest",
    slot: "HEAD",
    armorType: "leather",
    allowedClasses: ["rogue"],
    allowedSpecs: ["rogue-assassination", "rogue-outlaw", "rogue-subtlety"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { mastery: true, versatility: true },
    isTierPiece: true,
    setBonusKey: "motley-of-the-grim-jest",
    wowheadUrl: "https://www.wowhead.com/item=250006",
    sourceRefs: [wowheadItemRef(250006), rogueTierSetRef],
    variants: raidVariants(250006),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "도적 한밤 시즌1 티어 머리입니다. 보스별 드랍/변환 경로와 한국어 공식명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250004,
    nameKo: "암담한 재담의 맹독 보관통",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Venom Casks of the Grim Jest",
    slot: "SHOULDER",
    armorType: "leather",
    allowedClasses: ["rogue"],
    allowedSpecs: ["rogue-assassination", "rogue-outlaw", "rogue-subtlety"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { haste: true, mastery: true },
    isTierPiece: true,
    setBonusKey: "motley-of-the-grim-jest",
    wowheadUrl: "https://www.wowhead.com/item=250004",
    sourceRefs: [wowheadItemRef(250004), rogueTierSetRef],
    variants: raidVariants(250004),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "도적 한밤 시즌1 티어 어깨입니다. 보스별 드랍/변환 경로와 한국어 공식명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250009,
    nameKo: "암담한 재담의 광적인 치장",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Fantastic Finery of the Grim Jest",
    slot: "CHEST",
    armorType: "leather",
    allowedClasses: ["rogue"],
    allowedSpecs: ["rogue-assassination", "rogue-outlaw", "rogue-subtlety"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { crit: true, mastery: true },
    isTierPiece: true,
    setBonusKey: "motley-of-the-grim-jest",
    wowheadUrl: "https://www.wowhead.com/item=250009",
    sourceRefs: [wowheadItemRef(250009), rogueTierSetRef],
    variants: raidVariants(250009),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "도적 한밤 시즌1 티어 가슴입니다. 보스별 드랍/변환 경로와 한국어 공식명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250007,
    nameKo: "암담한 재담의 손재주",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Sleight of Hand of the Grim Jest",
    slot: "HANDS",
    armorType: "leather",
    allowedClasses: ["rogue"],
    allowedSpecs: ["rogue-assassination", "rogue-outlaw", "rogue-subtlety"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { crit: true, haste: true },
    isTierPiece: true,
    setBonusKey: "motley-of-the-grim-jest",
    wowheadUrl: "https://www.wowhead.com/item=250007",
    sourceRefs: [wowheadItemRef(250007), rogueTierSetRef],
    variants: raidVariants(250007),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "도적 한밤 시즌1 티어 손입니다. 보스별 드랍/변환 경로와 한국어 공식명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250005,
    nameKo: "암담한 재담의 칼날집",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Blade Holsters of the Grim Jest",
    slot: "LEGS",
    armorType: "leather",
    allowedClasses: ["rogue"],
    allowedSpecs: ["rogue-assassination", "rogue-outlaw", "rogue-subtlety"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { haste: true, versatility: true },
    isTierPiece: true,
    setBonusKey: "motley-of-the-grim-jest",
    wowheadUrl: "https://www.wowhead.com/item=250005",
    sourceRefs: [wowheadItemRef(250005), rogueTierSetRef],
    variants: raidVariants(250005),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "도적 한밤 시즌1 티어 다리입니다. 보스별 드랍/변환 경로와 한국어 공식명은 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250033,
    nameKo: "포식의 파괴자 유입구",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Devouring Reaver's Intake",
    slot: "HEAD",
    armorType: "leather",
    allowedClasses: ["demon-hunter"],
    allowedSpecs: ["demon-hunter-devourer"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { haste: true, mastery: true },
    isTierPiece: true,
    setBonusKey: "devouring-reavers-sheathe",
    wowheadUrl: "https://www.wowhead.com/item=250033",
    sourceRefs: [wowheadItemRef(250033), devourerTierSetRef],
    variants: raidVariants(250033),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "Devourer 악마사냥꾼 한밤 시즌1 티어 머리입니다. 한국어 공식명/획득 경로 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250031,
    nameKo: "포식의 파괴자 배출장갑",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Devouring Reaver's Exhaustplates",
    slot: "SHOULDER",
    armorType: "leather",
    allowedClasses: ["demon-hunter"],
    allowedSpecs: ["demon-hunter-devourer"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { crit: true, mastery: true },
    isTierPiece: true,
    setBonusKey: "devouring-reavers-sheathe",
    wowheadUrl: "https://www.wowhead.com/item=250031",
    sourceRefs: [wowheadItemRef(250031), devourerTierSetRef],
    variants: raidVariants(250031),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "Devourer 악마사냥꾼 한밤 시즌1 티어 어깨입니다. 한국어 공식명/획득 경로 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250036,
    nameKo: "포식의 파괴자 동력장치",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Devouring Reaver's Engine",
    slot: "CHEST",
    armorType: "leather",
    allowedClasses: ["demon-hunter"],
    allowedSpecs: ["demon-hunter-devourer"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { crit: true, mastery: true },
    isTierPiece: true,
    setBonusKey: "devouring-reavers-sheathe",
    wowheadUrl: "https://www.wowhead.com/item=250036",
    sourceRefs: [wowheadItemRef(250036), devourerTierSetRef],
    variants: raidVariants(250036),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "Devourer 악마사냥꾼 한밤 시즌1 티어 가슴입니다. 한국어 공식명/획득 경로 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250034,
    nameKo: "포식의 파괴자 정수 손장갑",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Devouring Reaver's Essence Grips",
    slot: "HANDS",
    armorType: "leather",
    allowedClasses: ["demon-hunter"],
    allowedSpecs: ["demon-hunter-devourer"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { crit: true, haste: true },
    isTierPiece: true,
    setBonusKey: "devouring-reavers-sheathe",
    wowheadUrl: "https://www.wowhead.com/item=250034",
    sourceRefs: [wowheadItemRef(250034), devourerTierSetRef],
    variants: raidVariants(250034),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "Devourer 악마사냥꾼 한밤 시즌1 티어 손입니다. 한국어 공식명/획득 경로 추가 검수 전까지 확정 추천하지 않습니다.",
  },
  {
    itemId: 250032,
    nameKo: "포식의 파괴자 피스톤",
    nameKoVerified: true,
    nameKoVerifiedAt: "2026-06-19",
    nameKoSource: "blizzard",
    nameEn: "Devouring Reaver's Pistons",
    slot: "LEGS",
    armorType: "leather",
    allowedClasses: ["demon-hunter"],
    allowedSpecs: ["demon-hunter-devourer"],
    sourceType: "raid",
    sourceNameKo: "한밤 시즌1 레이드/티어 변환",
    sourceNameEn: "Midnight Season 1 Raid/Tier Catalyst",
    sourceRaidKey: "midnight-s1-raid",
    stats: { haste: true, versatility: true },
    isTierPiece: true,
    setBonusKey: "devouring-reavers-sheathe",
    wowheadUrl: "https://www.wowhead.com/item=250032",
    sourceRefs: [wowheadItemRef(250032), devourerTierSetRef],
    variants: raidVariants(250032),
    season: "midnight-s1",
    confidence: "medium",
    recommendationState: "needs_check",
    tags: ["needsSim"],
    note: "Devourer 악마사냥꾼 한밤 시즌1 티어 다리입니다. 한국어 공식명/획득 경로 추가 검수 전까지 확정 추천하지 않습니다.",
  },
];

export const midnightS1ItemLevelNotes = {
  mythicPlus: "쐐기 종료 보상과 위대한 금고 템렙은 실제 한밤 시즌1 테이블 확인 후 표시합니다.",
  raid: "공격대 난이도별 템렙은 검증 전까지 UI에서 확정값처럼 보여주지 않습니다.",
  crafted: "제작템은 품질, 문장, 불꽃, 제작 옵션에 따라 실제 템렙과 가치가 달라집니다.",
};

